...
def _prompt(text: str, sender_name: str | None = None) -> str:
    ...
    return (
        ...
        "Финансовая операция — это любое упоминание денег: продажи, покупки, оплаты, переводы, "
        "получение, списание, долг, возврат, зарплата, аренда, страховые договоры и т.д. "
        "Не используй жёсткий список ключевых слов — анализируй контекст. "
        "ВАЖНЫЕ ПРАВИЛА ДЛЯ СТРАХОВЫХ ОТЧЁТОВ:\n"
        "1. Формат 'НБ ИФЛ 1/4390' = 1 договор на 4390 рублей → income=4390, category='ИФЛ'\n"
        "2. Формат 'НБ ДФР 1/3000' = 1 договор на 3000 рублей → income=3000, category='ДФР'\n"
        "3. Формат '3/15,7' = 3 договора на 15.7 тыс рублей → income=15700, category по контексту\n"
        "4. Формат 'Итого: 136,62' = 136.62 тыс рублей → income=136620\n"
        "5. Если число с запятой (например 15,7) и контекст тыс.рублей — умножь на 1000\n"
        "6. Если просто число без единиц измерения в отчёте продаж — это тыс. рублей\n"
        "7. Категория = тип страхования (ИФЛ, ОСАГО, КАСКО, НС, ДМС, КФР, ДФР и т.д.)\n"
        "8. Расход = потратили деньги, Доход = получили/продали/собрали премию\n"
        "9. 'ОСАГО 86' = продано 86 тыс рублей → income=86000, category='ОСАГО'\n"
        "10. 'ОСАГО 0/12,9/+12,9' = план 0, факт 12.9, отклонение +12.9 → income=12900\n"
        "Если сообщение не содержит чисел или упоминания денег — это не финансовая операция. "
        "В поле explanation укажи цитату из сообщения. "
...
def _batch_prompt(messages: list[dict]) -> str:
    ...
    return (
        ...
        "Финансовая операция — любое упоминание денег: продажи, покупки, оплаты, страховые договоры и т.д. "
        "ПРАВИЛА ДЛЯ СТРАХОВЫХ ОТЧЁТОВ:\n"
        "1. 'НБ ИФЛ 1/4390' = 1 договор на 4390 руб → income=4390, category='ИФЛ'\n"
        "2. '3/15,7' = 3 договора на 15.7 тыс руб → income=15700\n"
        "3. 'Итого: 136,62' = 136.62 тыс руб → income=136620\n"
        "4. Число с запятой в отчёте продаж = тыс. рублей (умножь на 1000)\n"
        "5. Категория = тип страхования (ИФЛ, ОСАГО, КАСКО, НС, ДМС, КФР, ДФР)\n"
        "6. Доход = продали/собрали, Расход = потратили\n"
        "7. 'ОСАГО 86' = 86 тыс руб → income=86000\n"
        "Если сообщение не содержит чисел или денег — не финансовая операция. "
        "В explanation укажи цитату. Не придумывай суммы. "
        "Не смешивай разные message_id. "
        "Сообщения JSON: "
